...
class PyBulletSim:
    ...
    def move_tool(self, position, orientation, speed=0.03):
        ...
        target_joint_state = p.calculateInverseKinematics(self.robot_body_id, self.robot_end_effector_link_index, position, orientation, residualThreshold=1e-4, maxNumIterations=100)
        # ===============================
    ...
    def get_grasp_position_angle(self, object_id):
        ...
        grasp_angle = p.getEulerFromQuaternion(grasp_angle)[2] + np.pi / 2.0
        # ==================================
        return position, grasp_angle

    def robot_go_home(self, speed=0.03):
    ...
    def execute_grasp(self, grasp_position, grasp_angle):
        ...
        # 1. open gripper
        self.open_gripper()
        #print("openings the gripper")
        # 2. Move gripper to pre_grasp_position_over_bin
        self.move_tool(pre_grasp_position_over_bin,  gripper_orientation, speed=0.03)
        #print("moved to pre-grasp position over bin")
        # 3. Move gripper to pre_grasp_position_over_object
        self.move_tool(pre_grasp_position_over_object,  gripper_orientation, speed=0.03)
        #print("moved to pre-grasp position over object")
        # 4. Move gripper to grasp_position
        self.move_tool(grasp_position,  gripper_orientation, speed=0.03)
        #print("moved to grasp position")
        # 5. Close gripper
        self.close_gripper()
        #print("closed the gripper")
        # 6. Move gripper to post_grasp_position
        self.move_tool(post_grasp_position,  gripper_orientation, speed=0.03)
        #print("moved the gripper to post grasp position")
        # 7. Move robot to robot_home_configuration
        self.robot_go_home(speed=0.1)
        #print("go home position for the gripper")
